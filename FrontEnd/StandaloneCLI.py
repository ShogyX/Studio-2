...
def umbrellafunc (input_string):
    ...
    password_info = {key: value for key, value in password_info.items() if value}
    #password_info["Issues Found"] = issues_found
    return issues_found, password_info.keys()
...
def TestAgainstMultiple(some_list):
    print("Running Test Agiainst Multiple Passwords")
    for word in some_list:
        issues_in_password = umbrellafunc(word)
        print(f"Password: [{word}], Issues Found: [{issues_in_password[0]}], {list(issues_in_password[1])}")

TestAgainstMultiple(blend)